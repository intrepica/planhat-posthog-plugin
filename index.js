...
export function setupPlugin({ config, attachments, global, metrics }) {
  metrics.pluginLoads.increment(1);

  if (config.eventWhitelist) {
    global.eventWhitelist = new Set(config.eventWhitelist.split(","));
    console.info(`${global.eventWhitelist.size} events whitelisted`);
  }

  if (config.eventBlacklist) {
    global.eventBlacklist = new Set(config.eventBlacklist.split(","));
  ...
  if (attachments.userWhitelist) {
    try {
      ...
      if (attachedWhitelist.length > 0) {
        ...
        console.info(`${global.userWhitelist.size} users whitelisted`);
      }
    } catch (e) {
